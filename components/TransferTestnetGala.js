...
const TransferTestnetGala = ({ walletAddress, metamaskClient, isConnected }) => {
  ...
  const transferTokens = async () => {
    ...
    try {
      const transferTokensDto = {
        ...
        uniqueKey: `transfer-token-${Date.now()}`
      };

      const signedTransferDto = await metamaskClient.sign("TransferTokens", transferTokensDto);
      
      const response = await fetch(`${process.env.NEXT_PUBLIC_ASSET_TESTNET_TOKEN_CONTRACT}/TransferToken`, {
        method: 'POST',
...
export default TransferTestnetGala;
